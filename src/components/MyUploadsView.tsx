      ...
      <div className="grid gap-4">
        {filteredDocuments.length === 0 ? (
          ...
          filteredDocuments.map(ddoc => (
            <Card key={ddoc.id} className="shadow-md hover:shadow-lg transition-shadow">
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-slate-800 mb-2">{ddoc.title}</h3>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${getStatusColor(ddoc.status)}`}>
                      {getStatusText(ddoc.status)}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="text-sm text-slate-500">
                    {ddoc.school && `Trường: ${ddoc.school}`}
                    {ddoc.reviewedAt && (
                      <div className="mt-1">
                        {ddoc.status === 'approved' ? 'Duyệt' : 'Từ chối'}: {new Date(ddoc.reviewedAt).toLocaleDateString('vi-VN')}
                      </div>
                    )}
                  </div>
                  <div className="flex gap-2">
                    {ddoc.fileContent && (
                      <Button
                        ...
                        onClick={() => window.open(ddoc.fileContent as string, '_blank')}
                      >
                ...
                {ddoc.reviewNote && (
                  <div className="mt-4 p-3 bg-slate-50 rounded text-sm">
                    ...
                    <div className="text-slate-600">{ddoc.reviewNote}</div>
                  </div>
                )}
              </div>
            </Card>
